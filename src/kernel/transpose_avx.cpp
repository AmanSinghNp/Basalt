...
void transpose_tiled_avx(const float* src, float* dst,
    ...
    // Keep cache tile at least one AVX micro-kernel wide.
    if (block_size < 8) {
        block_size = 8;
    }

    // Two-level blocking:
    //   cache tile (block_size) -> 8x8 AVX micro-kernel.
    for (size_t block_i = 0; block_i < rows; block_i += block_size) {
        const size_t i_end = std::min(block_i + block_size, rows);
        const size_t i_avx_end = block_i + ((i_end - block_i) / 8) * 8;

        for (size_t block_j = 0; block_j < cols; block_j += block_size) {
            const size_t j_end = std::min(block_j + block_size, cols);
            const size_t j_avx_end = block_j + ((j_end - block_j) / 8) * 8;

            // AVX core for full 8x8 regions inside current cache tile.
            for (size_t bi = block_i; bi < i_avx_end; bi += 8) {
                for (size_t bj = block_j; bj < j_avx_end; bj += 8) {
                    transpose_8x8_avx(
                        src + bi * cols + bj,
                        dst + bj * rows + bi,
                        cols,
            ...
            // Right edge inside cache tile (partial columns).
            for (size_t i = block_i; i < i_avx_end; ++i) {
                for (size_t j = j_avx_end; j < j_end; ++j) {
                    dst[j * rows + i] = src[i * cols + j];
                }
            }

            // Bottom edge inside cache tile (partial rows).
            for (size_t i = i_avx_end; i < i_end; ++i) {
